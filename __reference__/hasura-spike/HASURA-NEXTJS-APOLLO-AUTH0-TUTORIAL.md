...
The fastest way to try out Hasura is via [Hasura Cloud](https://hasura.io/cloud/). [Hasura Cloud](https://hasura.io/cloud/) gives you a scalable, highly available, globally distributed, fully managed, secure GraphQL API as a service!

See [https://hasura.io/learn/graphql/nextjs-fullstack-serverless/hasura-backend/](https://hasura.io/learn/graphql/nextjs-fullstack-serverless/hasura-backend/) for details on how to setup your free [Hasura Cloud](https://hasura.io/cloud/) account with a free PostgreSQL database server on [Heroku](https://www.heroku.com/) with [Heroku Postgres](https://www.heroku.com/postgres).
...
Now let's apply the migrations defined in `./migrations`:

```sh
$ npx hasura migrate apply
```

Great! Now navigate to the [Hasura Console](http://localhost:8080/console).
...
function (user, context, callback) {
  ...
  const url = "https://5ab4f6eaa674.ngrok.io/v1/graphql";
...
2. Unzip and make sure you're in the app directory
...
const createHttpLink = (headers) => {
  const httpLink = new HttpLink({
    uri: `https://${process.env.GRAPHQL_ENDPOINT}`,
    credentials: "include",
...
const createWSLink = () => {
  return new WebSocketLink(
    new SubscriptionClient(`wss://${process.env.GRAPHQL_ENDPOINT}`, {
      lazy: true,
      reconnect: true,
...
To see these changes in context, please refer to `app/components/Todo/TodoInput.js`
...
Please see `app/components/OnlineUsers/OnlineUsersWrapper.js` for how this was implemented, and for how we removed the mock placeholder state and content.
